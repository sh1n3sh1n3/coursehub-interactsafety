                ...
                <div class="col-lg-12">
                    <div class="ibox ">
						...
						<?php function checkimg($pic){
								if (file_exists("../assets/images/category/" . $pic)) {
									return false;
								} else {
									return true;
								}
                    					 ...
                    					 if (in_array($file_extension, $validextensions)) {
                    						 if ($_FILES["image"]["error"] > 0){
                    							...
                    							$up=0;
                    							  ...
                    							  $targetPath = "../assets/images/category/".$image; // Target path where file is to be stored
							...
							$insert = $conn->query("UPDATE category SET title= '".$title."', showHome= '".$showHome."', slug= '".$slug."', image= '".$image."', `description`= '".$_POST['description']."', price='".$_POST['price']."', delivery_types='".$_POST['delivery_types']."' WHERE id=".$id);
							if($insert){
						...
						$testimonial = $conn->query("SELECT * FROM category WHERE id=".$_GET['id'])->fetch_assoc();
						?>
