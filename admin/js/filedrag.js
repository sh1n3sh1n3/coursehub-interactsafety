...
(function() {
	...
	function ParseFile(filedata) {
            var image_type = filedata.type;
                if((image_type == 'image/png') || (image_type == 'image/jpeg') || (image_type == 'image/gif')){
                    var image_name = filedata.name;
    //                Output(image_type);
                    var reader1 = new FileReader();
                    reader1.onload = function(filedata) {
                    var src = filedata.target.result;
//                    console.log(src);
                     var html = '<div class="uploading_image">\n\
                                        <div id="image" class="img-thumbnail-original light-disable">\n\
                                            <img alt="..." src="'+src+'" style="width:120px;height:150px; padding-bottom:30px;" class="img-thumbnail" />\n\
                                            <div class="remove_image_div">\n\
                                                <a class="btn btn-xs btn-default" title="Remove Image" onclick="remove_image;"><i class="glyphicon glyphicon-trash text-danger"></i></a>\n\
                                            </div>\n\
                                        </div>\n\
                                    </div>';
                    $('#uploading_image_div').append(html);$('<input>').attr({
                        type: 'hidden',
                        class: 'images',
                        value: src,
                        name: 'pro_images[]'
                }).appendTo('#create_look-cont');
