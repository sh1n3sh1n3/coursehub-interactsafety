...
$id = $_SESSION['admin'];
$select1 = $conn->query("SELECT * FROM admin WHERE id = '".$id."'")->fetch_assoc();?>
<!DOCTYPE html>
<html>
<head>
                    ...
                    <div class="ibox ">
						...
						<?php $err=$msg='';
							if(isset($_POST['update'])) {
								...
								if(isset($_POST['old_pass']) && isset($_POST['new_pass']) && !empty($_POST['old_pass'])&& !empty($_POST['new_pass'])) {
									...
									if($old_pass == $select1['password']){
										$update = $conn->query("UPDATE admin SET password = '".$new_pass."',pass1 = '".$new_pass1."',name = '".$name."', mobile = '".$phone."',email = '".$email."' WHERE id=".$id);
										if($update){
									...
									$update = $conn->query("UPDATE admin SET name = '".$name."', mobile = '".$phone."',email = '".$email."' WHERE id=".$id);
									if($update){
										$msg = 'Your data has been updated successfully.';
									}else {
										$err = $conn->error;
							...
							<?php if(!empty($err)){
									...
									$select = $conn->query("SELECT * FROM admin WHERE id = '".$id."'")->fetch_assoc();?>
                            ...
                            <div>
								...
								<form class="account-frm" method="POST">
									...
									<div class="form-group  row"><label class="col-sm-2 col-form-label">Full Name</label>
										<div class="col-sm-10"><input type="text" required class="form-control" name="name" value="<?php echo $select['name']; ?>"></div>
									</div>
									<div class="hr-line-dashed"></div>
									...
									<div class="form-group  row"><label class="col-sm-2 col-form-label">Old Password</label>
										<div class="col-sm-10"><input type="text" class="form-control" name="old_pass" value=""></div>
									</div>
									<div class="hr-line-dashed"></div>
									<div class="form-group  row"><label class="col-sm-2 col-form-label">New Password</label>
										<div class="col-sm-10"><input type="text" class="form-control" name="new_pass" value=""></div>
									</div>
									<div class="hr-line-dashed"></div>
