...
function callme(dataURL) {
    ...
    $.ajax({
       url: url+"submitSign.php",
       type: "POST",
       data:  {courseid:courseid, locid:locid, slotid:slotid, teacherid:teacherid, userid:userid, type:type, sign: dataURL},
       beforeSend: function(){
            $("#myloader").css("display","block");
            $("#myloader").css("background","url("+url+"images/LoaderIcon.gif) center / 75px 75px no-repeat");
            $("#Sabeuplabtn").prop("disabled", true);
        },
       success: function(data) { console.log(data);
           if(data == '1') { 
              alert("Attandance submit successfully.");
              window.location.href=url+"account.php"
           } else {
              alert("Something wrong. Please try again.");
              $("#Sabeuplabtn").prop("disabled", false);
           }
